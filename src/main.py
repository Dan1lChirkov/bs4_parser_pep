...
def pep(session):
    ...
    for pep in tqdm(tr):
        pep_count += 1
        status_short = find_tag(pep, 'td').text[1:]
        if status_short in EXPECTED_STATUS:
            status_long = EXPECTED_STATUS[status_short]
        else:
            status_long = []
            logging.info(
                f'В строке {pep} неверно указан статус {status_short}'
            )
        pep_short_link = find_tag(pep, 'a')['href']
        pep_full_link = urljoin(PEP_URL, pep_short_link)
        response = get_response(session, pep_full_link)
        if response is None:
            return
        soup = BeautifulSoup(response.text, 'lxml')
        pep_info_table = find_tag(
            soup, 'dl', attrs={'class': 'rfc2822 field-list simple'}
        )
        pep_info_details = pep_info_table.find_all('dt')
        for detail in pep_info_details:
            if 'Status' in detail.text:
                status_page = detail.find_next_sibling().text
                break
        if status_page not in status_long:
            logging.info(
                f'Несовпадающие статусы: {pep_full_link}'
